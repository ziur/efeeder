var 
	...
	G_FORCE_STRENGTH = 0.0005 / G_ITERATIONS_PER_TICK,
	G_INITIAL_SPIN_FACTOR = Math.pow(0.96, 1 / G_ITERATIONS_PER_TICK),
	G_SLOWING_FACTOR = Math.pow(0.9, 1 / G_ITERATIONS_PER_TICK),
	G_DECAY_TO_CENTER = 0.01 / G_ITERATIONS_PER_TICK,
	PI_2 = Math.PI * 2.0;
...
{
	var PARTICLE_DIAMETER_2 = this.ParticleRadius * this.ParticleRadius * 4;
	var decayToCenter = G_DECAY_TO_CENTER / (this.remaining * this.remaining);
	...
	for (var j = 0; j < total; ++j)
	{
		...
		{
			pj.dx += -pj.x * decayToCenter;
			pj.dy += -pj.y * decayToCenter;
			pj.dz += -pj.z * decayToCenter;
			pj.dx *= G_SLOWING_FACTOR;
			pj.dy *= G_SLOWING_FACTOR;
			pj.dz *= G_SLOWING_FACTOR;
		}
	}
...
ParticleSystem.prototype.select = function(x, y)
{
	var PARTICLE_DIAMETER_2 = this.ParticleRadius * this.ParticleRadius * 4;
	...
	var total = this.particles.length;
	var selected = this.maxParticles;
	for (var i = 0; i < total; ++i)
	{
		var p = this.particles[i];
		var dx = p.x - x;
...
ParticleSystem.prototype.moveSelected = function(x, y)
{
	if (this.selected >= this.particles.length) return;
	
	x = (x - this.X0) / this.scale;
	y = (y - this.Y0) / this.scale;	
	this.particles[this.selected].x = x;
	this.particles[this.selected].y = y;
}
...
var g_lastExpell = g_oldTick + 1600;
function _updateFrame()
{
	...
	if (particleSystem)
	{
		...
		if ((currentTick - g_lastExpell) > (2000 / particleSystem.maxParticles))
		{
...
function start()
{
	...
	CANVAS.onmousemove = function (e)
	{
		...
		if (1 == g_mouse.button)
		{
			particleSystem.moveSelected(x, y);
		}
	...
	{
		var rect = CANVAS.getBoundingClientRect();
		g_mouse.x0 = e.clientX - rect.left;
		g_mouse.y0 = e.clientY - rect.top;
		g_mouse.x = g_mouse.x0;
		g_mouse.y = g_mouse.y0;
		g_mouse.button = e.which;
		g_mouse.action = 0;
		
		if (1 == g_mouse.button)
		{
			particleSystem.select(g_mouse.x, g_mouse.y);
		}
